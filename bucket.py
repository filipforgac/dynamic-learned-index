...
class Bucket:
    ...
    def insert(self, data: Tensor, ids: list[int]) -> None:
    ...
    def search(self, query: Tensor, k: int, distance_metric: str) -> tuple[Tensor, Tensor]:
        return search_vectors(self.vectors, self.ids, query, k, distance_metric)
...
def search_vectors(
    ...
    D, I_local = faiss.knn(query_np, vectors, k, metric=metric)
    I_global = ids[I_local]
